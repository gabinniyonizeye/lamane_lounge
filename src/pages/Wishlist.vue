  ...
  <div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-6xl mx-auto px-4">
      ...
      <div v-if="wishlist.products?.length === 0" class="text-center py-12">
        ...
        <router-link to="/shop" class="text-amber-600 hover:text-amber-700 font-semibold">
          Continue Shopping
        </router-link>
      </div>

      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div v-for="item in wishlist.products" :key="item.productId._id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
          <!-- Image -->
          <img
          ...
          <!-- Content -->
          <div class="p-4">
            <h3 class="font-semibold mb-2">{{ item.productId.name }}</h3>
            ...
            <div class="flex gap-2">
              <button
                ...
                class="flex-1 bg-amber-600 text-white py-2 rounded hover:bg-amber-700"
              >
                Add to Cart
              </button>
              <button
