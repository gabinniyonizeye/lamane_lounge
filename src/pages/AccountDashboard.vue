<template>
  <div class="min-h-screen bg-white py-12">
    <div class="max-w-6xl mx-auto px-4">
      <h1 class="text-4xl font-bold mb-8 text-gray-900">My Account</h1>
      ...
      <!-- Profile Tab -->
      <div v-if="activeTab === 'Profile'" class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <h2 class="text-2xl font-bold mb-4 text-gray-900">Profile Information</h2>
        <div class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium mb-1 text-gray-900">Phone</label>
              <p class="text-gray-700">{{ user.phone || 'Not set' }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium mb-1 text-gray-900">Birthday</label>
      ...
      <!-- Wishlist Tab -->
      <div v-if="activeTab === 'Wishlist'" class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <h2 class="text-2xl font-bold mb-4 text-gray-900">Saved Items</h2>
        <div v-if="wishlist.products?.length === 0" class="text-gray-500">
          No items in wishlist
        </div>
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div v-for="item in wishlist.products" :key="item.productId._id" class="border border-gray-200 rounded-lg p-4 bg-white">
            <img :src="item.productId.image" :alt="item.productId.name" class="w-full h-32 object-cover rounded mb-2" />
            <p class="font-semibold text-gray-900">{{ item.productId.name }}</p>
            <p class="text-amber-600 font-bold">{{ item.productId.price.toLocaleString() }} RWF</p>
            <button
              @click="removeFromWishlist(item.productId._id)"
              class="mt-2 w-full bg-red-100 text-red-600 py-1 rounded hover:bg-red-200 transition"
            >
              Remove
            </button>
          </div>
      ...
      <!-- Addresses Tab -->
      <div v-if="activeTab === 'Addresses'" class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <h2 class="text-2xl font-bold mb-4 text-gray-900">Saved Addresses</h2>
        <div v-if="addresses.length === 0" class="text-gray-500 mb-4">
          No saved addresses
        </div>
        <div v-else class="space-y-3 mb-4">
          <div v-for="(addr, idx) in addresses" :key="idx" class="border border-gray-200 rounded-lg p-3 bg-white">
      ...
      <!-- Settings Tab -->
      <div v-if="activeTab === 'Settings'" class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <h2 class="text-2xl font-bold mb-4 text-gray-900">Account Settings</h2>
        <div class="space-y-4">
          ...
          <label class="flex items-center gap-3">
            ...
            <span class="text-gray-900">Subscribe to Newsletter</span>
          </label>
          <button class="bg-amber-600 text-white px-6 py-2 rounded hover:bg-amber-700 transition">
            Save Settings
          </button>
